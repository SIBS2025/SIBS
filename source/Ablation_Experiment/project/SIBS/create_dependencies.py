...
def run_config_and_clean(file_name, build_dir):
    ...
    for idx, char in enumerate(file_name):
        if char == "-":
            ...
            #New feature: copy config.h into the configs folder and rename it
            source_config = os.path.join(build_dir, "src/config.h")
            configs_dir = os.path.join(build_dir, "configs")
            os.makedirs(configs_dir, exist_ok=True) 
            id = file_name.split("--")[0][1:].strip()
            target_config = os.path.join(configs_dir, f"config{id}.h")
            try:
